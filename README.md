...
- **Cut duplicate network calls.** Every dashboard route was calling
  Supabase's `auth.getUser()` twice per page load (once in the shared
  layout, once again in the page) — a real round-trip to the auth server,
  not a free check. Wrapped it in React's request-level `cache()` so it
  only happens once per page load across the whole app.
- **Parallelized independent database queries.** The dashboard, progress,
  and settings pages were fetching habits, logs, and profile data one after
  another even though none of them depend on each other. They now run
  together via `Promise.all`, cutting server-side data-fetch time roughly
  in half to a third on those pages.
- **One Supabase client per browser tab, not one per component.** The
  dashboard renders 10+ habit cards; each used to create its own Supabase
  client. Now they share a single instance.
- **Smaller JS bundles** — `optimizePackageImports` in `next.config.ts`
  makes sure Framer Motion, recharts, and lucide-react only ship the exact
  pieces each page actually uses instead of the whole library.
- **Loading skeletons** for Dashboard, Progress, and Journal, so the page
  shows its shape immediately while data streams in, instead of a blank
  screen.
